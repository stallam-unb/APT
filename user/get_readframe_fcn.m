...
elseif strcmpi(ext,'.mat'),
  ...
  %catching case where try to load a .mat file without any videofiletype info associated.
  if isempty(fieldnames(videofiletype))         
      %testing to see if the matlab file contains variables locs &
      %contents.  If so, treating it as a 'heatmap' file generated by poseNet
      fileContents = who('-file', filename);      
      if ismember('scores', fileContents) && ismember('locs', fileContents)
        videofiletype = 'poseNetHeatmap';
      else
        videofiletype = 'notVideo';
      end      
  end
  ...
  switch videofiletype,
    ...
    case 'SingleLarvaTracker',
     ...
     case 'poseNetHeatmap' %not a video but a heatmap generated by poseNet. Heatmap is in the variable 'scores'
        HMmatObj = matfile(filename); %heatmap is too big to load into memory. This is matlab object pointing at heatmap file
        scoresInfo = whos(HMmatObj,'scores');
        nframes = scoresInfo.size(1);
        fid = 0;
        nr = scoresInfo.size(2);
        nc = scoresInfo.size(3);
        hmPt2use = 1; %scores contains heatmaps for all points. Can only display one at time.  This specifies which point.  Initializing for point 1.
        readframe = @(f,hmPt2use) squeeze(HMmatObj.scores(f,:,:,hmPt2use)); 
        headerinfo = struct('nr',nr,'nc',nc,'nframes',nframes,'pointDisplayed',hmPt2use,'type','poseNetHeatmap');
