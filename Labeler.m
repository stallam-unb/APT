    ...
    function hlpSetCurrPrevFrame(obj,frm,tfforce)
      ...
      if obj.currFrame~=frm || tfforce
        ...
        for iView=1:obj.nview
            ...
            %if reading in heatmaps generated by poseTF, provide hmPt2use
            %input to movieReader() specifying which point to make heat map for
            if strcmp(obj.movieReader(iView).info.type,'poseNetHeatmap')
                obj.currIm{iView} = obj.movieReader(iView).readframe(frm,'hmPt2use',obj.hmPt2use,...
                    'doBGsub',obj.movieViewBGsubbed);
            else % if doing normal video reading just provide normal inputs
