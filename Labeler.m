...
% Bransonlab Animal Video Labeler/Tracker
  ...
  properties (Constant,Hidden)
    ...
    SAVEPROPS = { ...
      ...
      'labeledpos' 'labeledpostag' 'labeledposTS' 'labeledposMarked' 'labeledpos2' 'rawImportedPred' ...
  ...
  properties (SetObservable)
    ...
    rawImportedPred;      % same as labeledpos2 except that hasn't gone through post-processing - extracted from _3d_res.mat aux file generated by poseNet
    hideRawImpPred=1;        % 1[default] if want to hide rawImportedPred, 0 if want to show them
...
%       end
      ...
      if ~isfield(s,'rawImportedPred') %
          s.hideRawImpPred=1;
          for iMov = 1:numel(s.labeledpos)
                try
                    s.rawImportedPred{iMov} = nan(s.cfg.NumLabelPoints*s.cfg.NumViews, 2, s.movieInfoAll{iMov,1}.nframes);
                catch
                    s.rawImportedPred{iMov} = nan(10,2,1455);
                    warning('Failed to initialize rawImportedPred correctly')
                end
          end
      end
        ...
        for iVw = 1:nView
          ...
          if tfTag
            lpostag(iPt,frmsTrkIB,iTgtsIB) = s.pTrkTag(:,tfInBounds,tfiTgtIB);
                    ...
                    %If it exists, loading raw (pre-reprojection) predictions from
          %accompanying _3Dres.mat file
          ...
          %Both currently (14/12/18) true but may not be in the future!
          if iVw ==1 %if first view loading _3dres.trk
              ...
              if ~exist(threeDfname,'file') %if no 3D file found setting 3D data to NaN
                  v1raw = nan(2,nPhysPts,size(s.pTrk,3));
                  v2raw = nan(2,nPhysPts,size(s.pTrk,3));
              else
                  threeDresData = load(threeDfname);
                  %loading raw (before post processing) predictions
                  v1raw = threeDresData.psidebest; %view 1 raw predictions (no post-processing) [xy,PhysicalPoint,frame]  [2,nPhysPts,nFrms]
                  v2raw = threeDresData.pfrontbest; %[xy,PhysicalPoint,frame]  [2,nPhysPts,nFrms]
                  disp(['Loaded raw predictions from ', threeDfname])
              end   
              obj.rawImportedPred{iMov} = [permute(v1raw,[2,1,3]);permute(v2raw,[2,1,3])]; %putting in same [#physicalPoints*nViews,xy,frames] format as post-processed predictions
          end
...
%     end
    ...
    function labelsMiscInit(obj)
      ...
      %hard coding raw imported prediction labels to have same colors as
      %.labeledpos2 but markersize half as big and colors half as bright.  Fix if necessary
      impPredRawPI = ptsPlotInfo;
      impPredRawPI.MarkerSize = round(impPredRawPI.MarkerSize/2);
      impPredRawPI.Colors = impPredRawPI.Colors./2;
      obj.genericInitLabelPointViz('impPredRaw_ptsH','labeledpos2_ptsTxtH',...
        obj.gdata.axes_curr,impPredRawPI);
    ...
    function labels2VizUpdate(obj)
      ...
      %displaying raw imported predictions (.labeledpos2 but with no post-processing, loaded from _3dres.mat file compainion file)
      if obj.hideRawImpPred ==0
           
           impPredRaw = obj.rawImportedPred{iMov}(:,:,frm);

           %forcing raw predictions to half half dimmer and half smaller markers -very crude way to do this - fix
            for pt=1:length(obj.impPredRaw_ptsH)
                obj.impPredRaw_ptsH(pt).Color = obj.labeledpos2_ptsH(pt).Color./3;
                obj.impPredRaw_ptsH(pt).MarkerSize = round(obj.labeledpos2_ptsH(pt).MarkerSize./2);
            end

            LabelCore.setPtsCoordsStc(impPredRaw,obj.impPredRaw_ptsH,...
            obj.labeledpos2_ptsTxtH,txtOffset);
      else
            LabelCore.setPtsCoordsStc(nan(size(obj.rawImportedPred{iMov}(:,:,frm))),obj.impPredRaw_ptsH,...
            obj.labeledpos2_ptsTxtH,txtOffset);
      end
