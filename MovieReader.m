...
% Like VideoReader, but wraps get_readframe_fcn
  ...
  methods
    ...
    function [im,imOrigType] = readframe(obj,i,varargin)
      ...
      [doBGsub,hmPt2use] = myparse(varargin,...
        'doBGsub',false,...
        'hmPt2use',NaN ... %hmPt2use = point user wants a poseTF heatmap plotted for,  NaN if no heatmaps used
        );      
      ...
      if ~isnan(hmPt2use) %special case when reading non-video heatmaps generated by poseTF
            im = obj.readFrameFcn(i,hmPt2use); %specify point want heatmap for as well as frame
      else %otherwise, normal video reading
